...
def _EvalSetDir(root, name):
  """Resolve a standard MTME test-set directory or this vendored WMT23 name."""
  standard = os.path.join(root, name)
  if os.path.isdir(standard):
    return standard
  if name == 'wmt23':
    vendored = os.path.join(root, 'wmt23_data')
...
class EvalSet:
  ...
  def Ratings(self, rating_name) -> dict[str, list[Any | None]]:
  ...
  def _ReadDataset(
    ...
    if read_stored_ratings:
      raise ValueError(
          'Stored human-rating loading is not included in this vendored MTME'
      )
    ...
    if read_stored_metric_scores:
      for md in metric_scores_paths:
        md = os.path.join(_EvalSetDir(md, name), 'metric-scores', lp)
...
def CompareMetrics(
  ...
  Args:
    ...
    perm_test: Permutation test to use, either 'scores' or 'pairs' to select
      ...
      KendallWithTiesOpt.
    parallel_file: Unsupported in this vendored build. Leave as None to run
      significance comparisons sequentially.
...
def CompareMetricsWithPairwiseConfidenceError(
  ...
  # Compute metric correlations, ordered by decreasing correlation.
  corrs_and_ranks = {}
  num_metrics = len(metric_corrs)
  if progress_callback:
    progress_callback(f'PCE point estimates for {num_metrics} metrics')
  for metric_index, (m, c) in enumerate(metric_corrs.items(), start=1):
    if progress_callback:
      progress_callback(
          f'PCE point estimate {metric_index}/{num_metrics}: {m}')
...
def ComputeSigMatrix(
  ...
  if parallel_file is not None:
    raise ValueError(
        'parallel_file is unavailable because Apache Beam is not vendored'
    )
  ...
  if not k:
    if progress_callback:
      progress_callback('significance resampling disabled (k=0)')
    return sig_matrix, draws_index, np.asarray(draws_list)

  num_pairs = n * (n - 1) // 2
  if progress_callback:
    progress_callback(
        f'significance testing {num_pairs} metric pairs with up to {k} draws')
  ...
  for i in range(n):
    for j in range(i + 1, n):
      ...
      if progress_callback:
        progress_callback(
            f'significance pair {pair_index}/{num_pairs}: '
            f'{metrics[i]} vs {metrics[j]}')
